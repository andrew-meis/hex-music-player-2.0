...
const toSyncItem = ($data: Prism<any>) => {
  ...
  return {
    ...
    server: $data.get('Server').get(0).get('$').transform(toServer).value,
    status: $data.get('Status').get(0).get('$').transform(toStatus).value,
    mediaSettings: $data.get('MediaSettings').get(0).get('$').transform(toMediaSettings).value,
    policy: $data.get('Policy').get(0).get('$').transform(toPolicy).value,
    location: $data.get('Location').get(0).get('$').transform(toLocation).value,
  };
};
...
const toSyncList = ($data: Prism<any>) => {
  ...
  return {
    ...
    device: $data.get('Device').get(0).transform(toDevice),
    syncItems: $data.get('SyncItems').get(0).get('SyncItem').toArray().map(toSyncItem),
  };
};
