...
const toProfile = ($data: Prism<any>): Profile => ({
  ...
  defaultSubtitleAccessibility: $data.get<number>('defaultSubtitleAccessibility').value,
...
export interface User {
  ...
  queueEmail: string;
  queueUid: unknown;
  rememberExpiresAt: Date | undefined;
  restricted: boolean;
  roles: string[];
...
const toUser = ($data: Prism<any>): User => ({
  ...
  profile: $data.get<Record<string, any>>('profile').transform(toProfile).value,
  ...
  rememberExpiresAt: $data.get<number>('rememberExpiresAt').transform(toDateFromSeconds).value,
  ...
  services: $data.get('services').toArray().map(toService),
  subscription: $data.get<Record<string, any>>('subscription').transform(toUserSubscription).value,
  subscriptionDescription: $data.get<string>('subscriptionDescription').value,
  subscriptions: $data.get('subscriptions').toArray().map(toSubscription),
