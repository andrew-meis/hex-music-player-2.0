...
const isAlbum = (x: any): x is Album => x._type === 'album';
...
const toAlbum = ($data: Prism<any>): Album => ({
  ...
  addedAt: $data.get<number>('addedAt').transform(toTimestamp).transform(toDate).value,
  ...
  librarySectionID: $data.get<string>('librarySectionID', { quiet: true }).value,
  librarySectionKey: $data.get<string>('librarySectionKey', { quiet: true }).value,
  ...
  viewedLeafCount: $data.get<number>('viewedLeafCount', { quiet: true }).value,
...
const toAlbumContainer = ($data: Prism<any>): AlbumContainer => {
  ...
  return {
    ...
    albums: $data.get('Metadata', { quiet: true }).toArray().map(toAlbum),
