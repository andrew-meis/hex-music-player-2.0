...
const toDevice = ($data: Prism<any>): Device => {
  ...
  return {
    ...
    connections: $data.get('Connection', { quiet: true }).toArray().map(toConnection),
    ...
    uri: `server://${$prop.get('clientIdentifier').value}/com.plexapp.plugins.library`,
    createdAt: $prop.get('createdAt', { quiet: true }).transform(toDateFromSeconds).value,
    lastSeenAt: $prop.get('lastSeenAt', { quiet: true }).transform(toDateFromSeconds).value,
    ...
    httpsRequired: $prop.get('httpsRequired', { quiet: true }).transform(toBoolean).value,
    ...
    natLoopbackSupported: $prop.get('natLoopbackSupported', { quiet: true }).transform(toBoolean)
      .value,
    publicAddressMatches: $prop.get('publicAddressMatches', { quiet: true }).transform(toBoolean)
      .value,
...
const toDeviceList = ($data: Prism<any>): Device[] => {
  ...
  return $data.get('Device').toArray().map(toDevice);
};
